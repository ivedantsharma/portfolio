    ...
    >
      ...
      {type === "textarea" ? (
        <textarea
          ...
          className="w-full bg-transparent text-white p-4 rounded-lg focus:outline-none resize-none relative z-10"
          rows={4}
        />
      ) : (
        <input
          ...
          onBlur={() => setIsFocused(false)}
          placeholder={placeholder}
          className="w-full bg-transparent text-white p-4 rounded-lg focus:outline-none relative z-10"
        />
      )}
    </motion.div>
  );
...
const CosmicButton = ({
  ...
  return (
    <motion.button
      ref={buttonRef}
      className="relative w-full overflow-hidden rounded-lg bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-6 font-bold text-lg"
      variants={variants}
      initial="default"
...
const SpaceshipTerminal = () => {
  ...
  const socialLinks = [
    {
      ...
      icon: Linkedin,
      color: "#0077B5",
      link: "http://linkedin.com/in/vedantsharma10",
    },
    {
      name: "GitHub",
      icon: Github,
      color: "#333",
      link: "https://github.com/ivedantsharma",
    },
    {
      name: "Email",
      icon: Mail,
    ...
    },
    {
      ...
      icon: Instagram,
      color: "#E1306C",
      link: "https://instagram.com/ivedant_sharma",
    },
    {
      name: "Medium",
      icon: BookHeart,
      color: "#2962FF",
      link: "https://medium.com/@ivedantsharma",
    },
  ];
  ...
  return (
    <motion.div
      ref={terminalRef}
      className="w-full h-[370px] md:h-[500px] bg-gray-900 rounded-lg overflow-hidden relative"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <div className="relative z-10 p-6 md:p-8 h-full flex flex-col">
        <div className="flex justify-between items-center mb-6">
          <div className="md:text-xs text-[10px] text-blue-400 font-mono">
        ...
        <div className="flex-1 grid grid-cols-3 gap-4">
          {socialLinks.map((social) => (
            <motion.a
              ...
              onHoverStart={() => setActiveButton(social.name)}
            ...
            >
              ...
              <div className="relative z-10 h-full flex flex-col items-center justify-center p-4 bg-gray-800 rounded-lg border border-blue-500 overflow-hidden">
                <motion.div
                  ...
                  initial={false}
                  animate={
                    activeButton === social.name
                      ? { scale: [1, 1.5, 1], opacity: [0, 0.2, 0] }
                      : {}
                  }
                  transition={{ duration: 0.5 }}
                />
                <social.icon className="md:w-8 md:h-8 w-5 h-5 text-blue-400 mb-2" />
                <span className="md:text-xs text-[10px] text-blue-300 font-mono">
...
export const Contact = () => {
  ...
  return (
    <motion.section
      ref={containerRef}
      id="contact"
      className="min-h-screen mx-auto max-w-screen-2xl pt-5 md:py-5 lg:py-10 relative overflow-hidden"
      style={{ opacity }}
    >
      <ParticleField>
            ...
            >
              <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
                ...
                <CosmicInput
                  name="message"
                  type="textarea"
                  placeholder="Your Message to Maverick"
                  value={formState.message}
                  onChange={handleChange}
                />
                <CosmicButton onClick={() => handleSubmit}>
                  {isSubmitting
                    ? "Transmitting..."
                    : isSubmitted
...
const Notification = ({
  ...
  type: { color: string; message: string };
}) => {
  return (
    <motion.div
      className={`bg-${type.color}-500 text-white p-4 rounded-lg text-center absolute right-1 lg:right-4 bottom-96 lg:bottom-[450px]`}
