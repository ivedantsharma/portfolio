    ...
    <section className="min-h-screen max-w-screen-2xl mx-auto mb-10" id="blogs">
      ...
      <ul className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:pt-10">
        {blogs.slice(0, 6).map((blog) => (
          <li
            key={blog.node.title}
            className="bg-minor-project-card rounded-2xl p-4"
          >
            ...
            <div className="flex flex-col p-5">
              <h2 className="text-lg text-project-card-text font-semibold">
                {blog.node.title}
              </h2>
              <p className="text-gray-600 mt-2">
                {truncateWords(blog.node.brief, 20)}
              </p>
              <a
                href={blog.node.url}
                target="_blank"
                rel="noreferrer"
                className="text-accent inline-flex items-center gap-2 mt-2"
              >
